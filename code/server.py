...
def encrypt_data(data, key):
    ...
    return iv + ct  # IV is prepended to ciphertext
...
def encrypt():
    ...
    expiry = int(request.form['expiry'])  # seconds from now
...
def decrypt():
    ...
    print("QR raw data:", repr(data))  # Debug print
    if not data:
        print("No QR code found in image.")
        return jsonify({'error': 'No QR code found'}), 400
    lines = data.split('\n')
    print("QR lines:", lines)  # Debug print
    try:
        salt = base64.urlsafe_b64decode(lines[0][5:])
        encrypted = base64.urlsafe_b64decode(lines[1][5:])
        expiry = int(lines[2][4:])
        now = int(time.time())
        print("salt:", salt)
        print("encrypted:", encrypted)
        print("expiry:", expiry, "now:", now)
        if now > expiry:
